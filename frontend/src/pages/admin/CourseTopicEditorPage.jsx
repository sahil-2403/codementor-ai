  ...
  return <PageShell className="space-y-5 pb-8">
    ...
    <form onSubmit={handleSubmit(submit)} className="mx-auto w-full max-w-3xl space-y-5">
      <Card className="space-y-4 shadow-sm">
        {courseLocked ? <><input type="hidden" {...register('course')} /><Select label="Course" value={courseId} disabled><option value={courseId}>{selectedCourse?.title || topicQuery.data?.topic?.course?.title || 'Selected course'}</option></Select></> : <Select label="Course" {...register('course')} error={errors.course?.message}><option value="">Select course</option>{courses.map((course) => <option key={course._id} value={course._id}>{course.title}</option>)}</Select>}
