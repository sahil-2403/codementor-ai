...
export default function CourseQuestionEditorPage({ bank = 'quiz' }) {
  ...
  useEffect(() => {
    if (bank !== 'quiz' || editing) return;
    const lessons = lessonsQuery.data?.lessons || [];
    const relatedLesson = questionQuery.data?.question?.relatedLesson?._id || '';
    if (relatedLesson && !lessons.some((lesson) => lesson._id === relatedLesson)) setValue('relatedLesson', '');
  }, [bank, editing, lessonsQuery.data?.lessons, questionQuery.data?.question?.relatedLesson?._id, setValue]);
  ...
  if (editing && questionQuery.error) {
    return <EmptyState title="Question is unavailable" description={questionQuery.error.message} actionLabel={`Back to ${meta.path}`} onAction={() => navigate(`/admin/questions/${meta.path}`)} />;
  }

  if (editing && questionQuery.data?.question?.bank !== bank) {
    return <EmptyState title="Question bank mismatch" description="This question belongs to a different question bank." actionLabel="Back to questions" onAction={() => navigate('/admin/questions')} />;
  }
  ...
  return (
    <PageShell className="space-y-5 pb-8">
      <PageHeader
        ...
        title={editing ? `Edit ${meta.title.toLowerCase()}` : `Create ${meta.title.toLowerCase()}`}
        description={meta.helper}
        actions={<Link to={courseId ? `/admin/questions/${meta.path}?course=${courseId}` : `/admin/questions/${meta.path}`} className="ui-button ui-button--secondary gap-2"><ArrowLeft size={16} /> Back</Link>}
      />
      ...
      <form onSubmit={handleSubmit(submit)} className="space-y-5">
        <Card className="mx-auto w-full max-w-4xl space-y-4 shadow-sm">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Select label="Topic" disabled={!courseId} {...register('topic')} error={errors.topic?.message}>
          ...
          {questionType === 'code_output' ? (
            <FormTextarea label="Code snippet" rows={7} registration={register('codeSnippet')} error={errors.codeSnippet?.message} placeholder="Paste the code the learner should evaluate..." />
          ) : null}

          {questionType !== 'short_answer' ? (
            <FormInput label="Answer options" registration={register('optionsText')} error={errors.optionsText?.message} placeholder="Option A, Option B, Option C" />
          ) : null}
          ...
          {bank === 'quiz' ? (
            <Select label="Related lesson" disabled={!topicId} {...register('relatedLesson')} error={errors.relatedLesson?.message}>
              <option value="">No related lesson yet (draft only)</option>
              {lessons.map((lesson) => <option key={lesson._id} value={lesson._id}>{lesson.title}{lesson.status !== 'published' ? ' — draft' : ''}</option>)}
            </Select>
          ) : (
            <div className="rounded-surface border border-primary/15 bg-primary-soft/35 p-3 text-sm leading-6 text-muted-foreground">
              Skill checks are intentionally not linked to Lessons. They assess the learner's current Course knowledge before roadmap generation.
            </div>
          )}
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="secondary" onClick={() => navigate(courseId ? `/admin/questions/${meta.path}?course=${courseId}` : `/admin/questions/${meta.path}`)}>Cancel</Button>
          <Button type="submit" isLoading={mutation.isPending} loadingLabel="Saving question...">{editing ? 'Save changes' : 'Create question draft'}</Button>
        </div>
      </form>
    </PageShell>
  );
