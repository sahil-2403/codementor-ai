...
export default function MentorPage() {
  ...
  return <PageShell>
    <PageHeader eyebrow="Learning mentor" title="CodeMentor" description="Ask questions about your current lesson, recent mistakes, or a concept you want to understand better." actions={<Badge variant={aiAvailable ? 'info' : 'warning'}>{aiAvailable ? 'Live help available' : 'Saved explanations'}</Badge>} />
    ...
    {suggestionsQuery.error && <InlineAlert tone="danger" title="Suggested questions are unavailable">{suggestionsQuery.error.message} <button type="button" className="font-semibold underline" onClick={() => suggestionsQuery.refetch()}>Try again</button></InlineAlert>}
    {(providerNotice || !aiAvailable) && <InlineAlert tone="warning" title="Live mentor responses are unavailable">{providerNotice || 'You can still open saved explanations from your lessons.'}</InlineAlert>}

    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <div className="min-w-0 space-y-5">
        <ChatWindow messages={messages} isResponding={askMutation.isPending} emptyMessage={aiAvailable ? 'Choose a suggested question or ask your first coding question.' : 'Choose a saved explanation from the guidance panel.'} />
        ...
        <form onSubmit={handleSubmit((values) => sendPayload({ text: values.message, type: values.promptType }))} className="rounded-panel border border-border bg-surface p-4 shadow-soft">
          <div className="grid gap-4 md:grid-cols-[220px_minmax(0,1fr)]">
            <FormSelect label="Answer style" error={errors.promptType?.message} {...register('promptType')} disabled={!aiAvailable || askMutation.isPending}>
              <option value="freeform">Direct answer</option><option value="simple_explanation">Simple explanation</option><option value="real_project_example">Project example</option><option value="interview_answer">Interview answer</option><option value="practice_question">Practice question</option><option value="revision_notes">Revision notes</option>
            </FormSelect>
            <label className="block space-y-1.5"><span className="ui-field-label">Your question</span><textarea className="ui-field-control min-h-28 resize-y" placeholder={aiAvailable ? 'Describe the concept, code, or mistake you want help with.' : 'Live questions are unavailable. Choose a saved explanation.'} aria-invalid={Boolean(errors.message)} disabled={!aiAvailable || askMutation.isPending} {...register('message')} />{errors.message && <span className="ui-field-error">{errors.message.message}</span>}</label>
          </div>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-muted-foreground">Your current lesson and recent practice may be used to make the answer more relevant.</p><Button type="submit" disabled={!aiAvailable} isLoading={askMutation.isPending} loadingLabel="Preparing response...">Send question</Button></div>
        </form>
      </div>
      ...
      <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start" aria-label="Mentor guidance">
        ...
        <Card><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 shrink-0 text-success" size={20} aria-hidden="true" /><div><h2 className="font-bold text-foreground">Relevant learning context</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">Answers can use your current roadmap, lesson, recent quiz mistakes, and topics that need more practice.</p></div></div></Card>
        <Card><div className="flex items-start gap-3"><BookOpenCheck className="mt-0.5 shrink-0 text-primary" size={20} aria-hidden="true" /><div><h2 className="font-bold text-foreground">Saved explanations</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">These explanations come from your lesson material and remain available when live mentor responses are unavailable.</p></div></div></Card>
      </aside>
    </div>
  </PageShell>;
}
