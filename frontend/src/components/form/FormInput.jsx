...
export default function FormInput({ label, error, registration, className = '', ...props }) {
  return <label className="block space-y-1.5">
    ...
    <input
      className={cn('w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100', className)}
      {...registration}
      {...props}
    />
