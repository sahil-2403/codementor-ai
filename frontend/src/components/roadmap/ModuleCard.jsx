...
const moduleVisuals = [
  ...
  { match: /project/i, icon: FolderCode, className: 'bg-blue-50 text-blue-700 ring-blue-100' },
...
export default function ModuleCard({ module, index = 0, isLast = false, defaultExpanded = false }) {
  ...
  const visual = getModuleVisual(module.title);
  ...
  return (
    <article className="relative grid grid-cols-[36px_minmax(0,1fr)] gap-3 sm:grid-cols-[44px_minmax(0,1fr)] sm:gap-4">
      ...
      <section
        className={cn(
          'overflow-hidden rounded-panel border bg-surface shadow-sm transition duration-200',
          completedModule && 'border-success/20',
          inProgress && !completedModule && 'border-primary/30 shadow-soft',
      ...
      >
        ...
        >
          <div className="flex items-start gap-3 sm:gap-4">
            <span className={cn('grid h-11 w-11 shrink-0 place-items-center rounded-surface ring-1 sm:h-12 sm:w-12', visual.className)} aria-hidden="true">
              <ModuleIcon size={20} />
            </span>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="min-w-0">
                  ...
                  <h3 className="mt-1 text-base font-bold text-foreground sm:text-lg">{module.title}</h3>
              ...
              <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
                <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-surface-secondary" role="progressbar" aria-label={`${module.title} completion`} aria-valuemin="0" aria-valuemax="100" aria-valuenow={completion}>
                  <div
          ...
          <div id={contentId} className="border-t border-border/80 bg-surface">
            ...
            <div className="flex flex-col gap-3 border-t border-border/80 bg-surface-secondary/35 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">Module assessment</p>
                  <p className="mt-0.5 text-xs leading-5 text-muted-foreground">Test what you learned when a quiz is available for this module.</p>
                </div>
              </div>
              ...
              {locked ? (
                ...
                <Link to={`/quizzes/${module._id}`} className="ui-button ui-button--secondary min-h-9 shrink-0 px-3.5 text-xs">
                  <ClipboardCheck size={15} aria-hidden="true" /> Take module quiz
                </Link>
              ) : (
