...
export default function ProjectSubmissionFeedback({ submission }) {
  ...
  return <div className={`mt-4 rounded-panel border p-5 ${fallback ? 'border-warning/20 bg-warning-soft' : 'border-primary/20 bg-primary-soft'}`}>
    {fallback && <InlineAlert tone="warning" title="Detailed review unavailable" className="mb-4">Your submission is saved. This checklist has no score, does not add practice topics, and does not change your progress.</InlineAlert>}
    <p className="font-semibold leading-7 text-foreground">{feedback.summary}</p>
    <div className="mt-4 grid gap-4 md:grid-cols-2">
      {!fallback && feedback.strengths?.length ? <div><h4 className="font-bold text-success">What you did well</h4><ul className="mt-2 space-y-2 text-sm text-muted-foreground">{feedback.strengths.map((item) => <li key={item} className="flex gap-2"><span aria-hidden="true">•</span><span>{item}</span></li>)}</ul></div> : null}
      {feedback.improvements?.length ? <div><h4 className="font-bold text-error">What to improve</h4><ul className="mt-2 space-y-2 text-sm text-muted-foreground">{feedback.improvements.map((item) => <li key={item} className="flex gap-2"><span aria-hidden="true">•</span><span>{item}</span></li>)}</ul></div> : null}
    </div>
