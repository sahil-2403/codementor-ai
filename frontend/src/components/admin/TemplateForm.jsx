...
const emptyForm = { goalKey: 'junior-mern-stack', level: 'beginner', title: '', description: '', estimatedDurationDays: 90, status: 'draft', modulesText: JSON.stringify(defaultModules, null, 2) };
...
export default function TemplateForm({ initialData = null, onSubmit, onCancel, isLoading }) {
  ...
  return <form onSubmit={handleSubmit(submit)} className="grid gap-3 rounded-[2rem] bg-white/70 p-5">
    ...
    <div className="grid gap-3 md:grid-cols-2"><Input label="Estimated days" type="number" error={errors.estimatedDurationDays?.message} {...register('estimatedDurationDays')} /><Select label="Status" {...register('status')}><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></Select></div>
