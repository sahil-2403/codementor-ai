...
import { ArrowDown, ArrowUp, BookOpenText, CheckCircle2, CircleAlert, Clock3, Layers3, Plus, Trash2 } from 'lucide-react';
  ...
  return (
    <label className={`flex cursor-pointer items-start gap-3 rounded-surface border border-transparent px-3 py-2.5 transition hover:border-border hover:bg-surface ${disabled ? 'cursor-not-allowed opacity-45' : ''}`}>
      ...
      <span className="min-w-0 flex-1">
        ...
        {meta ? <span className={`mt-0.5 block break-words text-xs leading-5 ${warning ? 'text-warning' : 'text-muted-foreground'}`}>{meta}</span> : null}
      </span>
    </label>
  );
}
...
export default function TemplateForm({
  ...
  const {
    ...
    defaultValues: toForm(initialData, courseId)
  });
  const modules = useFieldArray({ control, name: 'modules' });
  const moduleValues = useWatch({ control, name: 'modules' }) || [];

  useEffect(() => {
    reset(toForm(initialData, courseId));
  }, [initialData, courseId, reset]);
    ...
    <form onSubmit={handleSubmit(submit)} className="space-y-5">
      ...
      <Card className="shadow-sm">
        <FormSectionHeader
          ...
          title="Define the course roadmap"
          description="The template belongs to one course and one learner level. Content from another course cannot be selected."
        />
        <div className="mt-5 space-y-4">
          <div className="rounded-panel border border-border bg-surface-secondary/45 p-4">
            ...
            <p className="mt-1 break-words text-base font-bold text-foreground">{course?.title || 'Select a course first'}</p>
          </div>
          <div className="grid gap-4 md:grid-cols-[1.4fr_0.6fr]">
            <Input
              label="Template title"
              placeholder="Example: React Developer — Beginner"
            ...
            <Select label="Level" error={errors.level?.message} {...register('level')}>
              {COURSE_LEVELS.map((level) => {
                ...
                const disabledForCourse = !(course?.availableLevels || []).includes(level);
                return (
                  <option key={level} value={level} disabled={unavailable || disabledForCourse}>
                    {level[0].toUpperCase() + level.slice(1)}
                    {unavailable ? ' — already configured' : disabledForCourse ? ' — disabled for course' : ''}
                  </option>
                );
              })}
            </Select>
      ...
      <Card className="shadow-sm">
        <FormSectionHeader
          ...
          description="Each card is one module. Lesson choices and Quiz-bank tags are limited to the selected course."
          action={
            <Button
              type="button"
              variant="secondary"
              ...
              <article key={field.id} className="rounded-panel border border-border bg-surface-secondary/25 p-4 sm:p-5">
                <div className="flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">Card position defines the learner order automatically.</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="ghost" className="min-h-9 w-9 px-0" disabled={index === 0} onClick={() => modules.move(index, index - 1)} aria-label={`Move module ${index + 1} up`}><ArrowUp size={15} /></Button>
                    <Button type="button" variant="ghost" className="min-h-9 w-9 px-0" disabled={index === modules.fields.length - 1} onClick={() => modules.move(index, index + 1)} aria-label={`Move module ${index + 1} down`}><ArrowDown size={15} /></Button>
                    <Button type="button" variant="ghost" className="min-h-9 gap-1.5 px-3 text-xs text-error" disabled={modules.fields.length <= 1} onClick={() => modules.remove(index)}><Trash2 size={14} /> Remove</Button>
                  </div>
                </div>

                <div className="mt-4 grid gap-4 md:grid-cols-[1fr_160px]">
                  <Input label="Module title" placeholder="Example: Components and state" error={errors.modules?.[index]?.title?.message} {...register(`modules.${index}.title`)} />
                  <Input label="Duration (days)" type="number" min="1" max="90" error={errors.modules?.[index]?.durationDays?.message} {...register(`modules.${index}.durationDays`)} />
                </div>
                <div className="mt-4">
                  <Textarea label="Module description" rows={2} error={errors.modules?.[index]?.description?.message} {...register(`modules.${index}.description`)} />
                </div>

                <div className="mt-5 grid gap-4 xl:grid-cols-2">
                  <div className="min-w-0 rounded-panel border border-border bg-surface p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-foreground">Lessons</p>
                        <p className="mt-1 text-xs leading-5 text-muted-foreground">Select published or draft lessons from {course?.title || 'this course'}.</p>
                      </div>
                      <span className="shrink-0 rounded-full bg-primary-soft px-2.5 py-1 text-xs font-bold text-primary-strong">{selectedLessonCount}</span>
                    </div>
                    {lessonsByTopic.length ? (
                          ...
                          <div key={group.topic}>
                            ...
                            <div className="mt-1">
                              {group.lessons.map((lesson) => {
                                const usedElsewhere = lessonUsedElsewhere(lesson._id, index);
                                return (
                                  <ChoiceRow
                                    key={lesson._id}
                                    registration={register(`modules.${index}.lessons`)}
                                    value={lesson._id}
                      ...
                      <p className="mt-3 text-sm text-muted-foreground">No lessons are available in this course yet.</p>
                    )}
                  </div>
                  ...
                  <div className="min-w-0 rounded-panel border border-border bg-surface p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs leading-5 text-muted-foreground">Tags are collected only from Quiz-bank questions in this course.</p>
                      </div>
                      <span className="shrink-0 rounded-full bg-primary-soft px-2.5 py-1 text-xs font-bold text-primary-strong">{selectedTagCount}</span>
                    </div>
                    {quizTags.length ? (
                      <div className="mt-3 max-h-72 overflow-y-auto pr-1">
                        {quizTags.map((item) => (
                          <ChoiceRow
                            ...
                            value={item.tag}
                            title={item.tag}
                            meta={item.publishedCount > 0 ? `${item.publishedCount} published quiz question${item.publishedCount === 1 ? '' : 's'}` : 'Draft questions only'}
                            warning={item.publishedCount === 0}
                          />
                        ))}
                      </div>
                    ) : (
                      <p className="mt-3 text-sm text-muted-foreground">No Quiz-bank tags are available in this course yet.</p>
                    )}
                  </div>
                </div>
              </article>
      ...
      <Card className="shadow-sm">
        <FormSectionHeader
          ...
          title="Review real course dependencies"
          description="The server performs these checks again before publishing."
        />
        <div className="mt-5 grid gap-2 md:grid-cols-2">
          {readiness.map((item) => (
            <div
              key={item.label}
              className={`flex items-start gap-2 rounded-surface border px-3 py-3 text-sm ${item.ready ? 'border-success/20 bg-success-soft text-success' : 'border-warning/20 bg-warning-soft text-warning'}`}
            >
              {item.ready ? <CheckCircle2 size={17} className="mt-0.5 shrink-0" /> : <CircleAlert size={17} className="mt-0.5 shrink-0" />}
