...
const getRecentMistakes = async ({ userId, courseId }) => {
  ...
  return attempts.flatMap((attempt) => attempt.answers || [])
    .filter((answer) => !answer.isCorrect)
    .slice(0, 6)
    .map((answer) => ({ topic: answer.topic, selectedAnswer: answer.selectedAnswer, correctAnswer: answer.correctAnswer, explanation: answer.explanation }));
};

export const isRealAIAvailable = () => process.env.ENABLE_AI === 'true' && (process.env.AI_PROVIDER || 'mock') !== 'mock';
...
export const getMentorSuggestions = async ({ userId, lessonId }) => {
  ...
  if (lesson?.interviewQuestions?.length) {
    lesson.interviewQuestions.slice(0, 4).forEach((qa, index) => curatedQuestions.push({
      label: qa.question || `Question ${index + 1}`,
      promptType: 'saved_answer',
      text: qa.question || `Explain ${topic}`,
      answer: qa.answer || lesson.interviewDefinition || lesson.theory || 'Review the lesson notes and write one small example.'
    }));
  }
  ...
  if (!curatedQuestions.length) {
    curatedQuestions.push({ label: 'How should I study today?', promptType: 'saved_answer', text: 'How should I study today?', answer: 'Open your roadmap, continue the next available lesson, then complete one quiz or revision item. Use projects and interview mode after the core lesson is clear.' });
  }
  return {
...
export const askMentor = async ({ user, message, lessonId, promptType = 'freeform' }) => {
  ...
  if (!isRealAIAvailable()) {
    ...
    return { answer: '', chat: null, sources: [], contextSummary: { aiAvailable: false }, suggestedPrompts: suggestions.prompts, savedQuestions: suggestions.savedQuestions, aiAvailable: false, message: 'AI mentor is currently unavailable. Use saved explanations from your course content.' };
  }
  ...
  try {
    ...
    await logAIUsage({ user: user._id, feature: AI_FEATURES.MENTOR_CHAT, status: 'failed', model: process.env.AI_PROVIDER || 'mock', provider: process.env.AI_PROVIDER || 'mock', latencyMs: Date.now() - startedAt, promptFingerprint, contextSources: relatedContext.map((item) => item.source).filter(Boolean), metadata: { errorType: 'provider_failure', promptType }, errorMessage: error.message });
  }
  ...
  const contextSummary = {
    ...
    weakTopicCount: progress?.weakTopics?.length || 0,
  ...
  if (progress && lesson?.topic?.title) {
    await mergeWeakTopics({ progress, weakTopics: [{ topic: lesson.topic.title, score: 50 }], source: 'mentor_chat' });
  }
