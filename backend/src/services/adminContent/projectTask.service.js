...
const validateLessons = async ({ courseId, lessonIds = [], requirePublished = false }) => {
  ...
  const lessons = await Lesson.find({ _id: { $in: normalizedIds }, course: referenceId(courseId) }).select('_id status title').lean();
...
export const updateProjectTask = async ({ id, payload }) => {
  ...
  if (project.status === 'archived') throw new ApiError(409, 'Archived project tasks are read-only. Restore the project first.', [], 'CONTENT_ARCHIVED');
