...
const resolveTopicReference = async ({ topicRef, topicTitle, session = null }) => {
  ...
  if (!topic) {
    throw new ApiError(
      400,
      'Selected topic is unavailable',
      [{ field: 'topicRef', message: 'Choose an active topic' }],
...
const assertInterviewQuestionPublishable = async (question) => {
  ...
  if (!cleanStringArray(question.answerChecklist).length) {
    errors.push({ field: 'answerChecklist', message: 'Add at least one answer review point' });
  }
  if (errors.length) {
    throw new ApiError(400, 'Interview question is not ready to publish', errors, 'CONTENT_NOT_READY');
  }
...
export const resolveInterviewQuestionImpact = async (questionId, { session = null } = {}) => {
  const question = ensureFound(
    await withSession(
      InterviewQuestion.findById(questionId).populate('topicRef', 'title status'),
      session
    ),
    'Interview question'
  );
  const interviewAttempts = await withSession(
    InterviewAttempt.countDocuments({ question: question._id }),
    session
  );
...
export const listInterviewQuestions = async (query = {}) => {
  ...
  return listWithPagination({
    ...
    filter,
    query,
    populate: [{ path: 'topicRef', select: 'title status' }]
  });
};

export const getInterviewQuestion = async (id) => ensureFound(
  await InterviewQuestion.findById(id).populate('topicRef', 'title status'),
  'Interview question'
);
...
export const updateInterviewQuestion = async ({ id, payload }) => {
  ...
  const topicLink = hasTopicChange
    ? await resolveTopicReference({
        topicRef: payload.topicRef,
        topicTitle: payload.topic || question.topic
      })
    : {};
...
export const changeInterviewQuestionStatus = async ({ id, status, confirmPublish = false }) => {
  ...
  const result = await runLifecycleOperation(async (session) => {
    ...
    if (topicBlockers.length) {
      throw new ApiError(
        409,
        'Restore the parent topic before restoring this interview question.',
        [],
        'INTERVIEW_QUESTION_ARCHIVED_BY_TOPIC'
      );
    }
