...
export const submitQuiz = async ({ userId, moduleId, answers }) => {
  ...
  checkedAnswers.filter((answer) => !answer.isCorrect).forEach((answer) => {
    wrongByTopic.set(answer.topic, (wrongByTopic.get(answer.topic) || 0) + 1);
  });
...
export const explainQuizAttempt = async ({ user, attemptId }) => {
  ...
  try {
    ...
    aiResult = quizExplanationFallback({ attempt, relatedContext });
    aiResult.aiAvailable = false;
    await logAIUsage({ user: user._id, feature: AI_FEATURES.QUIZ_EXPLANATION, status: 'failed', model: process.env.AI_PROVIDER || 'mock', provider: process.env.AI_PROVIDER || 'mock', latencyMs: Date.now() - startedAt, promptFingerprint, contextSources: relatedContext.map((item) => item.source).filter(Boolean), metadata: { attemptId, errorType: 'provider_failure' }, errorMessage: error.message });
  }
  ...
  await logAIUsage({
    ...
    promptFingerprint,
    contextSources: attempt.aiExplanation.sources,
    metadata: { attemptId, score: attempt.score, wrongAnswerCount: wrongAnswers.length }
  });
