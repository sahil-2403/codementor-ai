...
const usageMetadata = { model: env.geminiModel, provider: 'gemini' };

export const sanitizeAIText = (value = '', maxChars = AI_SAFETY.MAX_MENTOR_PROMPT_CHARS) => collapseWhitespace(value).slice(0, maxChars);
export const sanitizeCodeText = (value = '', maxChars = AI_SAFETY.MAX_PROJECT_CODE_CHARS) => stripControlChars(value).trim().slice(0, maxChars);
const includesUnsupportedPattern = (text = '') => UNSUPPORTED_AI_PATTERNS.some((pattern) => pattern.test(text));

export const guardAIRequest = async ({ userId, feature, text, maxChars = AI_SAFETY.MAX_MENTOR_PROMPT_CHARS, minChars = 2, metadata = {} }) => {
  ...
  if (!cleaned || cleaned.length < minChars) {
    await logAIUsage({ user: userId, feature, status: 'blocked', ...usageMetadata, promptFingerprint: fingerprint, metadata: { ...metadata, reason: 'empty_or_too_short' } });
    throw new ApiError(400, 'Please enter a meaningful request.');
  }
  if (String(text || '').length > maxChars) {
    await logAIUsage({ user: userId, feature, status: 'blocked', ...usageMetadata, promptFingerprint: fingerprint, metadata: { ...metadata, reason: 'too_long', maxChars } });
    throw new ApiError(400, `AI request is too long. Maximum allowed characters: ${maxChars}.`);
  }
  if (includesUnsupportedPattern(cleaned)) {
    await logAIUsage({ user: userId, feature, status: 'blocked', ...usageMetadata, promptFingerprint: fingerprint, metadata: { ...metadata, reason: 'unsupported_prompt_pattern' } });
  ...
  const repeatedCount = await AIUsageLog.countDocuments({ user: userId, feature, promptFingerprint: fingerprint, createdAt: { $gte: since }, status: { $in: ['success', 'failed', 'blocked'] } });
  if (repeatedCount >= AI_SAFETY.REPEAT_LIMIT) {
    await logAIUsage({ user: userId, feature, status: 'blocked', ...usageMetadata, promptFingerprint: fingerprint, metadata: { ...metadata, reason: 'repeated_prompt', repeatedCount } });
