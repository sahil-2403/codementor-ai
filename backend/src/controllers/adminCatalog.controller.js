...
export const updateTechnologyStatus = asyncHandler(async (req, res) => sendResponse(res, 200, 'Technology status updated', { technology: await adminContent.changeTechnologyStatus({ id: req.params.id, ...req.body }) }));
export const deleteTechnology = asyncHandler(async (req, res) => sendResponse(res, 200, 'Technology permanently deleted', { technology: await adminContent.deleteTechnology(req.params.id) }));
...
export const updateCourseStatus = asyncHandler(async (req, res) => sendResponse(res, 200, 'Course status updated', { course: await adminContent.changeCourseStatus({ id: req.params.id, ...req.body }) }));
export const deleteCourse = asyncHandler(async (req, res) => sendResponse(res, 200, 'Course permanently deleted', { course: await adminContent.deleteCourse(req.params.id) }));
...
export const deleteLearningPath = asyncHandler(async (req, res) => sendResponse(res, 200, 'Learning path permanently deleted', { learningPath: await adminContent.deleteLearningPath(req.params.id) }));
